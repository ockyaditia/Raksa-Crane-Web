    ...
    <div class="page-content container-fluid">
      ...
      <div class="page-header-actions">
        <a  id="exampleToggle" class="btn btn-sm btn-default btn-outline btn-round">
          ...
          <span class="hidden-xs">Tambah Pembayaran</span>
        </a>
      </div>
    </div>
                ...
                <div id="examplePanel" class="panel margin-bottom-0 is-collapse is-close">
                 ...
                 <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <div class="input-group-btn">
                      <button name="tambah_pembayaran" id="submit" type="submit" class="btn btn-default btn-outline" tabindex="-1">Tambahkan</button>
                    </div>
                  </div>
					</form>
					<?php 
if(isset($_POST['tambah_pembayaran'])){
	 ...
	 $nomor_rekening = $_POST['nomor_rekening'];
		$insert_posts = "insert into pembayaran (nama_bank,cabang,nama_pemilik,nomor_rekening) values ('$nama_bank','$cabang','$nama_pemilik','$nomor_rekening')";
...
?> 
 ...
 <div class="panel">
            <header class="panel-heading">
              <h3 class="panel-title">Semua Pembayaran</h3>
            </header>
            <div class="panel-body">
              <table class="table table-bordered table-hover toggle-circle" id="exampleFootableFiltering">
                <thead>
                  <tr>
                    <th data-sort-ignore="true">Nama Bank</th>
					<th data-sort-ignore="true">Cabang</th>
					<th data-sort-ignore="true">Nama Pemilik</th>
					<th data-sort-ignore="true">Nomor Rekening</th>
                ...
                <tbody>
				<?php
	$get_cat = "select * from pembayaran";
	$run_cat = mysql_query($get_cat); 
	while ($row_cats = mysql_fetch_array($run_cat))
	{
			$id = $row_cats['id'];
			$nama_bank = $row_cats['nama_bank'];
			$cabang = $row_cats['cabang'];
			$nama_pemilik = $row_cats['nama_pemilik'];
			$nomor_rekening = $row_cats['nomor_rekening'];
	
		echo "
		<tr><form action='' method='post' enctype='multipart/form-data'>
                <td>
				<input type='hidden' name ='id' value='$id'>
				<input type='text' name='nama_bank' value='$nama_bank'></td>
				<td>
				<input type='text' name='cabang' value='$cabang'></td>
				<td>
				<input type='text' name='nama_pemilik' value='$nama_pemilik'></td>
				<td>
				<input type='text' name='nomor_rekening' value='$nomor_rekening'></td>
                <td>
				<button name='update_pembayaran' id='submit' type='submit' class='btn btn-icon btn-default btn-outline' data-placement='top' data-toggle='tooltip' data-original-title='Update'>
	...
	$update_categories = "update pembayaran set nama_bank='$nama_bank',cabang='$cabang',nama_pemilik='$nama_pemilik',nomor_rekening='$nomor_rekening' where id='$id'";
