...
<body>
  ...
  <div class="page animsition">
     ...
     <div class="panel">
            ...
            <div class="panel-body">
              <table class="table table-bordered table-hover toggle-circle" id="exampleFootableFiltering">
                ...
                <tbody>
				<?php
				$get_product = "select * from conversation order by rand() LIMIT 0,6";
	...
	while ($row_products = mysql_fetch_array($run_product))
	{
			...
			$conversation_status = $row_products['conversation_status'];
			$conversation_read = $row_products['conversation_read'];
			$i=1;
	$get_cat = "select * from messages WHERE conversation_id='$conversation_id'";
	$run_cat = mysql_query($get_cat) or trigger_error(mysql_error().$get_cat);
	...
	while ($row_cats = mysql_fetch_array($run_cat))
	{
			$i++;
				...
				echo "
				<tr>
                    <td>$conversation_title</td>
                    <td>$i</td>
					<td>$conversation_date</td>
					
                    ";
					
					if($conversation_read!=$i)
						{
						echo " 
							<td>
								<span class='label label-table label-warning'>Belum</span>
							</td> 
							";
						}
					else
						...
						echo " 
							<td>
								<span class='label label-table label-success'>Dibaca</span>
							</td> 
							";
						}
					echo "
					<td>$conversation_full_name</td>
                    <td>$conversation_email</td>
					<td>$conversation_phone_number</td>
					<td>$conversation_content</td>
					<td>
					<a href='view.php?id=$conversation_id' class='btn btn-icon btn-default btn-outline' data-placement='top' data-toggle='tooltip' data-original-title='Lihat Percakapan'>
							<span class='icon wb-chat-group' aria-hidden='true'></span>
						</a>
