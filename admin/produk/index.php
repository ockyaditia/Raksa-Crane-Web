...
<body>
  ...
  <div class="page animsition">
              ...
              <table class="table table-bordered table-hover toggle-circle" id="exampleFootableFiltering">
                <thead>
                  <tr>
					...
					<th data-hide="phone, tablet">Pabrikan</th>
					<th data-hide="phone, tablet">Model</th>
					<th data-hide="phone, tablet">Tahun</th>
					<th data-hide="phone, tablet">Harga</th>
					<th data-hide="phone, tablet">Kapasitas</th>
                    <th data-hide="phone, tablet">Boom</th>
                    <th data-hide="phone, tablet">Jib</th>
                ...
                <tbody>
				<?php
				$get_product = "select * from produk order by rand() LIMIT 0,6";
	...
	while ($row_products = mysql_fetch_array($run_product))
	{
			...
			$produk_kategori = $row_products['category_id'];
			$produk_tanggal = $row_products['produk_date'];
			...
			$produk_published = $row_products['produk_published'];
			$get_cat = "select * from categories WHERE cat_id='$produk_kategori'";
	$run_cat = mysql_query($get_cat); 
	while ($row_cats = mysql_fetch_array($run_cat))
	{
			$produk_kategori = $row_cats['cat_title'];
	}
				...
				<tr>
					...
					<td>$produk_tanggal</td>
                    <td>
						<span class='label label-table label-dark'>$produk_tipe</span>
					</td>
					<td>$produk_pabrikan</td>
                    <td>$produk_model</td>
					<td>$produk_tahun</td>
					<td>$produk_harga</td>
                    <td>$produk_kapasitas</td>
					<td>$produk_boom</td>
					<td>$produk_jib</td>
                    ";
					
					if($produk_published==1)
						{
						echo " 
							<td>
					...
					echo "
					<td>
						<a href='edit.php?produk_id=$produk_id' class='btn btn-icon btn-default btn-outline' data-placement='top' data-toggle='tooltip' data-original-title='Edit'>
					...
					{
					echo "
					<a href='edit.php?non_aktifkan=$produk_id' class='btn btn-icon btn-default btn-outline' data-placement='top' data-toggle='tooltip' data-original-title='Non Aktifkan'>
					...
					echo "
					<a href='edit.php?aktifkan=$produk_id' class='btn btn-icon btn-default btn-outline' data-placement='top' data-toggle='tooltip' data-original-title='Aktifkan'>
